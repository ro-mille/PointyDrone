...
class DirectionDetector:
    def __init__(self):
        ...
        if sys.version_info >= (3, 8):
            ...
            (major, minor, micro) = version('mediapipe').split('.')           
        elif sys.version_info >= (3,):
            import pkg_resources as pkg
            version = pkg.get_distribution('mediapipe').version
            (major, minor, micro) = version.split('.')
        else:
            print('Need python version at least 3')

        if int(minor) >= 8 and int(micro) >= 6: 
            self.mp_drawing_styles = mp.solutions.drawing_styles
            self.drawOverHands = lambda image, LM: \
                                    self.drawOverHandsStyled(image, LM)
        else:
            self.drawOverHands = lambda image, LM: \
                                    self.drawOverHandsNoStyle(image, LM)

    def drawOverHandsStyled(self, original_image, handLMs):
    ...
    def drawOverHandsNoStyle(self, original_image, handLMs):
        self.mp_drawing.draw_landmarks(
            original_image,
            handLMs,
            self.mp_hands.HAND_CONNECTIONS)
